...
StringMap gFF2PrecachedData;

methodmap FF2Protected < ArrayList
{
	public FF2Protected()
	{
		gFF2PrecachedData = new StringMap();
	...
	public FF2Save Request(int boss)
	{
		...
		for(int i = 0; i < this.Length; i++)
		{
			save = this.Get(i);
			if(!save) {
				LogError("index: %i - null FF2Save in FF2Protected!", i);
				delete view_as<StringMap>(this.Get(i, 1));
				this.Erase(i--);
				continue;
			}
	...
	public void Purge()
	{
		while(this.Length) {
			...
			delete view_as<FF2Save>(this.Get(0));
			this.Erase(0);
		}
		delete gFF2PrecachedData;
		delete this;
	}
}
...
public bool UTIL_FindCharArg(const FF2Save save, const char[] args, char[] res, int maxlen)
{
	...
	char abkey[24];
	char[] key = new char[124];
	FormatEx(key, 124, "%s/%s", plugin, ability);
	kv = BossKV[save.boss];
	kv.Rewind();
	...
	for(int ab = 1; ab <= MAXRANDOMS; ab++)
	{
		...
		kv.GetString("name", key, 124);
		if(!StrEqual(key, ability)) {
			kv.GoBack();
			continue;
		}
		
		kv.GetString("plugin_name", key, 124);
		if(plugin[0] && !StrEqual(key, plugin)) {
			kv.GoBack();
			continue;
		}
